...
#`{r echo=FALSE}
plot <- data %>% group_by(Sport, Sex) %>% filter(Season=='Summer') %>% summarize(Count=n()) %>% group_by(Sex) %>% top_n(10,Sport)
ggplot(plot, aes(x=Sport, y=Count, group=Sex, label=format(Count, big.mark=".", decimal.mark=","))) +
